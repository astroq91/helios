...
float fbm(in vec3 v) {
  ...
  for (int i = 0; i < k_octaves; i++) {
    value += amplitude * noise(v * frequency * smoothstep(0.0, 1.0, abs(sin(p_stats.time * 0.3))) * 0.9);
...
float scene(vec3 pos) {
  float sphere1 = sd_sphere(pos - vec3(1.0 + cos(p_stats.time), 0.7, 0.0), 0.6);
  float sphere2 = sd_sphere(pos + vec3(0.7, 0.5 + sin(p_stats.time)/2.0, 0.0), 0.8);
  return smoothmin(sphere1, sphere2, 0.8);
}

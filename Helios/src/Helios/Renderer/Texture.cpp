...
stbi_uc* load_image(const std::filesystem::path& path, int* width, int* height,
                    int* channels) {
    return stbi_load(
...
void Texture::init_cube_map(const CubeMapInfo& cube_map_info, VkFormat format) {
    ...
    texture_data[0] =
        load_image(cube_map_info.right, &tex_width, &tex_height, &tex_channels);
    texture_data[1] =
        load_image(cube_map_info.left, &tex_width, &tex_height, &tex_channels);
    texture_data[2] =
        load_image(cube_map_info.top, &tex_width, &tex_height, &tex_channels);
    ...
    for (int i = 0; i < 6; i++) {
        ...
        VkCommandBuffer command_buffer =
            VulkanUtils::begin_single_time_commands(context.device,
                                                    context.command_pool);
        void* data;
        vkMapMemory(context.device, staging_buffer->get_vk_memory(), 0,
                    image_size, 0, &data);
        memcpy(data, texture_data[i], static_cast<size_t>(image_size));
        vkUnmapMemory(context.device, staging_buffer->get_vk_memory());
        ...
        VulkanUtils::copy_buffer_to_image(
            command_buffer, staging_buffer->get_vk_buffer(),
            m_image->get_vk_image(), static_cast<uint32_t>(tex_width),
            static_cast<uint32_t>(tex_height), i);
        VulkanUtils::transition_image_layout(
            {.image = m_image->get_vk_image(),
             .old_layout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
             .new_layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
